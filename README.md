# Project Archive RAG — 다년도 연구과제 문서 아카이브 검색·질의응답 RAG 시스템 구현 
...
보안 제약이 있는 자료를 다루는 것을 전제로, 임베딩·생성 모두 로컬에서 진행.
`config.yaml`의 `provider`를 바꾸면 상용 API로 전환됨(코드 변경 없음).
...
. `src/`의 코드는 특정 과제에 종속되지 않는 범용 파이프라인.
